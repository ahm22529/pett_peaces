import 'package:pett_peaces/fetures/doctor/domain/entity/doctor-details_entity.dart';
import 'package:pett_peaces/fetures/doctor/domain/entity/doctor_entity.dart';

import 'data.dart';

class DoctoreResponse extends DoctorEntity {
  bool? status;
  int? statusCode;
  Data? data;
  String? message;

  DoctoreResponse({this.status, this.statusCode, this.data, this.message})
      : super(coatch: data?.data ?? [], lastpage: data?.meta?.lastPage ?? 0);

  factory DoctoreResponse.fromJson(Map<String, dynamic> json) {
    return DoctoreResponse(
      status: json['status'] as bool?,
      statusCode: json['status_code'] as int?,
      data: json['data'] == null
          ? null
          : Data.fromJson(json['data'] as Map<String, dynamic>),
      message: json['message'] as String?,
    );
  }

  Map<String, dynamic> toJson() => {
        'status': status,
        'status_code': statusCode,
        'data': data?.toJson(),
        'message': message,
      };
}
